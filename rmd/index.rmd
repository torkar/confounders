...
```{r, message=FALSE}
# Package names
packages <- c("simcausal", "ggplot2", "brms", "ggdag", "cmdstanr", "tipr", "broom", "broom.mixed", "tidyverse", "dagitty", "bayesfam")
...
# options(cmdstanr_write_stan_file_dir = "/Users/torkarr/Downloads/")
...
D <- D +
  ...
  # Generate Beta(mu, phi) with BA and H.
  # Note: We use rbeta_mean() which is a mean parameterization of Beta.
  node("P",
     distr = "rbeta_mean",
     phi = 4,
     mu = inv_logit_scaled(b_H * H + b_BA * BA))
...
# Could add more noise N(0,1), to d$P.
...
H_err <- qt(0.975, df = N - 1) * sd(H_hat)/sqrt(N)
BA_err <- qt(0.975, df = N - 1) * sd(BA_hat)/sqrt(N)

# Lower and upper CI
H_ci_l <- format(round(mean(H_hat) - H_err, 3), nsmall = 3)
H_ci_u <- format(round(mean(H_hat) + H_err, 3), nsmall = 3)
BA_ci_l <- format(round(mean(BA_hat) - BA_err, 3), nsmall = 3)
BA_ci_u <- format(round(mean(BA_hat) + BA_err, 3), nsmall = 3)
H_mu <- format(round(mean(H_hat), 3), nsmall = 3)
BA_mu <- format(round(mean(BA_hat), 3), nsmall = 3)

print(paste0("H=", b_H, ", while 95% CI for simulated mu is: ", H_mu, "[", H_ci_l, ",", H_ci_u, "]"))
print(paste0("BA=", b_BA, ", while 95% CI for simulated mu is: ", BA_mu, "[", BA_ci_l, ",", BA_ci_u, "]"))
# nice and tight
```

The simulation indicates (even when we add noise) that we can recover the parameters we set. Let's create a data set where we also have a confounder $C$.
...
```{r 2nd-sim, cache=TRUE, warnings=FALSE, message=FALSE}
N <- 250 # Sample size
...
D <- D +
  ...
  node("P",
    distr = "rbeta_mean",
    phi = 4,
    mu = inv_logit_scaled(b_H * H + b_BA * BA + b_C * C))

# Need to use inv_logit() since mu in Beta needs it when modelling
Dset <- set.DAG(D, vecfun  = c("inv_logit_scaled"))
...
# TODO:
# need to remove this...
# Worst case run with brms + rstan since sampler fails...
# sanity check and nudge if P == 1.0
d$P[d$P == 1] <- 1 - 0.001 # ffs fix this

# Could add more noise, e.g., N(0,1), to d$P.
# Since P is already logit it's easy:
# d$P <- 1/(1+exp(-rnorm(n = N, mean = d$P, sd = 1)))
...
for(i in 1:N) {
  ...
  # sanity check and nudge if P == 1.0
  # no need for a flipped zi model
  d[d$P == 1,] <- 1 - 0.001 # ffs this changes not only P
...
H_err <- qt(0.975, df = N - 1) * sd(H_hat)/sqrt(N)
BA_err <- qt(0.975, df = N - 1) * sd(BA_hat)/sqrt(N)
C_err <- qt(0.975, df = N - 1) * sd(C_hat)/sqrt(N)
...
BA_ci_l <- format(round(mean(BA_hat) - BA_err, 3), nsmall = 3)
BA_ci_u <- format(round(mean(BA_hat) + BA_err, 3), nsmall = 3)
C_ci_l <- format(round(mean(C_hat) - C_err, 3), nsmall = 3)
C_ci_u <- format(round(mean(C_hat) + C_err, 3), nsmall = 3)

# means for each parameter
H_mu <- format(round(mean(H_hat), 3), nsmall = 3)
BA_mu <- format(round(mean(BA_hat), 3), nsmall = 3)
C_mu <- format(round(mean(C_hat), 3), nsmall = 3)

print(paste0("H=", b_H, ", while 95% CI for simulated mu is: ", H_mu, "[", H_ci_l, ",", H_ci_u, "]"))
print(paste0("BA=", b_BA, ", while 95% CI for simulated mu is: ", BA_mu, "[", BA_ci_l, ",", BA_ci_u, "]"))
print(paste0("C=", b_C, ", while 95% CI for simulated mu is: ", C_mu, "[", C_ci_l, ",", C_ci_u, "]"))
```

Now let's see what we get when we don't include the confounder.

```{r 3rd-sim, cache=TRUE, warnings=FALSE, message=FALSE}
p <- get_prior(P ~ H + BA, family = Beta, data = d)
p[1,1] <- "normal(0,1)"
p[4,1] <- "normal(0,2)"
...
<!-- Remember to do coeff/(1-coeff) to transform Beta reg coef to OR, before tipping results -->
