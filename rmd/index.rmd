...
In [@feldtSFT23sysrev], the authors extract causal links from several primary studies and assemble all disparate evidence into a Unified Directed Acyclic Graph (UDAG). The purpose is to estimate the direct effect of Business Area ($BA$) on Productivity, ($P$), i.e., Business Area can be seen as a "treatment" and Productivity is the outcome.

The above UDAG is clearly a more involved example and contains two of the elemental constructs one can find in a DAG. Organization ($O$) is a confounder since it point to the treatment and the outcome. Hardware ($H$) is part of a pipe since $BA \rightarrow H \rightarrow P$. The lower part also contains several ancestors of the outcome. Additionally, we also see that there are no colliders in the UDAG. So, this seems to be a simple task. Let's see what we should condition on by providing our UDAG :
...
Let's do this systematically and overlay numbers on the plot to make it easier to make sure that we don't miss anything. First, we focus on the nine edges in the upper part of the UDAG since they are all connected to $BA$ directly or indirectly without needing to use $P$.
