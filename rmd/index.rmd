...
```{r, include=FALSE}
# Package names
packages <- c("ggplot2", "brms", "ggdag", "cmdstanr", "tipr", "broom",
              "broom.mixed", "tidyverse", "dagitty", "rethinking", "foreach",
              "doParallel", "bayesplot")
...
Let's design a generative model according to the above.
...
#
# TODO: Remember to do coeff/(1-coeff) to transform Beta reg coef to OR, before
# tipping results

registerDoParallel(detectCores() - 1)

# Set effects
# Confounder's effect on TS; vary this too c(-1.0, -0.5, 0.0, 0.5, 1.0)
b_CTS <- 1.0
# Confounder's effect on outcome; vary this too c(-1.0, -0.5, 0.0, 0.5, 1.0)
b_C <- 1.3
# TS's effect on outcome; vary this too (-0.5, -0.25, -0.1, 0.1, 0.25, 0.5)
b_TS <- 0.1

C <- rnorm(100)
TS <- rpois(100, 5 + b_CTS * C)
E <- rnorm(100, b_C * C + b_TS * TS)
df <- data.frame(C = C, TS = TS, E = E)

# Set weakly informative priors
# N(0,2) prior on intercept; still ~0.008 prob for absurd, ⍺ = {-5,5}, effects
# N(0,1) prior on β est; still ~0.005 prob for absurd, β = {-3,3}, effects
# Exponential(1) on σ; still ~0.005 prob for absurd, σ = 5, effects
...
# Simulate varying sample size `n` and `n_sim`
...
# returns A numeric vector of size `n_sim` rows and `n` columns.
sim <- function(n = 125, n_sim = 200,
  ...
  for (i in seq_len(nrow(g))) {
    ...
    l[[i]] <- foreach(j = n, .combine = cbind) %:%
      foreach(k = seq_len(n_sim), .combine = c) %dopar% {
      C <- rnorm(j) # confounder
      TS <- rpois(j, 5 + b_cts * C) # TS effect is 5

      E <- rnorm(j, b_c * C + b_ts * TS) # E affected by b_TS and b_C
...
n <- seq(10, 100, 5) # Sample sizes for the rand() functions
n_sim <- 200 # number of sim runs

# For β_ts we need to consider an effect size. Generally, for a coefficient β,
# effect sizes between 0.10–0.29 are said to be only small, effect sizes between
# 0.30–0.49 are medium, and effect sizes of 0.50 or greater are large.
#
...
g <- expand.grid(c(-1.0, -0.5, 0.5, 1.0), # β_cts
                 ...
                 c(-0.5, -0.3, -0.1, 0.1, 0.3, 0.5)) # β_ts!

res <- sim(n, n_sim, g)
stopImplicitCluster()

# Average over each column!
# mean / median / q25/75 l[c(1-9),]
# Use HPDI 0.5 & 0.9 and save values...

HPDI(res[[1]], prob = 0.95)
```


Next, we need to decide on different effect sizes of $TS \rightarrow E$, and then analyze what size a confounder, $C$, need to be to invalidate those effects. We can safely assume that $TS$ has a positive effect on $E$, i.e., the larger the team, the more effort spent.
