...
Above we see that $\beta_Z \approx 1.50$ (i.e., the confounder_outome_effect), thus we have an indication that this works both ways. Given that we have introduced to distinct concepts used to estimate unobserved/unknown confounders, we can now turn out attention to a more involved example.
...
However, remember, there's still a challenge in dealing with Edges 7 and 8, i.e., in these two cases we need to analyze how large a confounder needs to be to nullify the direct causal effect of $BA$ on $P$. Let's do just that.
