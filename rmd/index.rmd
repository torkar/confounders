...
N <- seq(10, 50, 5) # Sample sizes for sim
...
l <- foreach(i = N, .combine = rbind) %:% # rows
  ...
  foreach(j = 1:250, .combine = c) %dopar% { # columns
    Ni <- eval(i)
    C <- rnorm(Ni) # confounder
    TS <- rpois(Ni, b_CTS * C + 5) # effect of interest affected by C+5
    E <- rnorm(Ni, b_C * C + b_TS * TS) # E affected by b_TS
...
HPDI(l[2, 1:250], prob = 0.95)
stopImplicitCluster()
```


Next, we need to decide on different effect sizes of $TS \rightarrow E$, and then analyze what size a confounder, $C$, need to be to invalidate those effects. We can safely assume that $TS$ has a positive effect on $E$, i.e., the larger the team, the more hours spent.
