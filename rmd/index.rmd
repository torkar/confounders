...
```{r, message=FALSE}
# Package names
packages <- c("ggplot2", "brms", "ggdag", "cmdstanr", "tipr", "broom", "broom.mixed", "tidyverse", "dagitty", "bayesfam", "simcausal")
...
Let's design a generative model according to the above specification.
...
D <- D +
  ...
  node("H",
     distr = "rcat.b1",
     probs = (BA==1)*c(0.7,0.1,0.2) + (BA==2)*c(0.2,0.1,0.7) + (BA==3)*.(0.2,0.1,0.7)) + 
  # Generate Beta(x, \phi) with BA and H. Set phi = 4 for now.
  # Note: We use rbeta_mean() which is a mean parameterization of Beta.
  node("P",
     distr = "rbeta_mean",
     phi = 4,
     mu = inv_logit_scaled(beta_H * H + beta_BA * BA))

# Need to use inv_logit() since mu in Beta needs it when modelling
Dset <- set.DAG(D, vecfun  = "inv_logit_scaled")
...
m <- brm(P ~ H + BA, family = Beta, data = d, prior = p)
...
for(i in 1:N) {
  ...
  H_Hat[i] <- fixef(m)[1] + fixef(m)[2]
  BA_Hat[i] <- fixef(m)[1] + fixef(m)[3]
}

# Margin of error 0.95
err_H <- qt(0.975,df=N-1)*sd(H_Hat)/sqrt(N)
err_BA <- qt(0.975,df=N-1)*sd(BA_Hat)/sqrt(N)

# Lower and upper CI
H_ci_l <- round(mean(H_Hat) - err_H, 3)
H_ci_u <- round(mean(H_Hat) + err_H, 3)
BA_ci_l <- round(mean(BA_Hat) - err_BA, 3)
BA_ci_u <- round(mean(BA_Hat) + err_BA, 3)
H_mu <- round(mean(H_Hat), 3)
BA_mu <- round(mean(BA_Hat), 3)

print(paste0("H=", beta_H, " , while 95% CI for mu of H is: ", H_mu, "[", H_ci_l, ",", H_ci_u, "]"))
print(paste0("BA=", beta_BA, " , while 95% CI for mu of BA is: ", BA_mu, "[", BA_ci_l, ",", BA_ci_u, "]"))

# One can always try to add more noise N(0,1) to P.
# Since P is already logit() it's easy:
# d$P <- 1 / (1 + exp(-rnorm(n = N, mean = d$P, sd = 1)))
```
...
##### BA #####
# First, generate $BA$ variable
# Set effects for each of five categories in BA
a <- 0.07
b <- 0.1
c <- -0.15
d <- -0.03
...
# Five categories, each w/ 20% representation
BA <- sample(x=c("A","B", "C", "D", "E"), 
              size=n, replace=TRUE, prob=rep(1/5, 5))

# Design matrix
m_BA <- as.matrix(dummy_cols(BA)[,-1])

# Bake in the effects we set above
pred_BA <- cbind(1, m_BA %*% c(a, b, c, d, e))

##### H #####
# Next generate $H$ variable
# Set effects for each of five categories in BA
a <- -0.5
b <- 0.15
c <- -0.25
...
# Five categories, each w/ 20% representation
H <- sample(x=c("A","B", "C", "D", "E"), 
              size=n, replace=TRUE, prob=rep(1/5, 5))

# Design matrix
m_H <- as.matrix(dummy_cols(H)[,-1])

# Bake in the effects we set above
pred_H <- cbind(1, m_H %*% c(a, b, c, d, e))

##### Generate outcome #####
# It's a Beta we're aiming for so "mu" uses logit link

# this is basically P <- BA and P <- H
# but we need BA -> H -> P and BA -> P
pr <- inv_logit_scaled(pred_BA[,2] + pred_H[,2])

# Use mean parameterization for Beta
P <- rbeta_mean(n, pr, 1)

# Put all data in data frame
d <- data.frame(P = P, BA = BA, H = H)

p <- get_prior(bf(P ~ BA + H, phi ~ 1), data = d, family = Beta)
p[1,1] <- "normal(0,1)"
p[10,1] <- "normal(0,2)"
p[11,1] <- "normal(0,2)"
m <- brm(bf(P ~ BA + H, phi ~ 1), data = d, prior = p, family = Beta, refresh = 0)

# A one shot give us this...
# Population-Level Effects: 
#               Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# Intercept        -0.33      0.22    -0.76     0.11 1.00     2874     3090
# phi_Intercept     0.07      0.07    -0.08     0.22 1.00     5370     2936
# BAB              -0.50      0.24    -0.96    -0.02 1.00     3587     3282
# BAC              -0.45      0.24    -0.91    -0.00 1.00     3426     3136
# BAD              -0.37      0.24    -0.83     0.11 1.00     3448     3327
# BAE               0.41      0.24    -0.07     0.89 1.00     3550     2945
# HB                0.93      0.24     0.44     1.40 1.00     3451     3332
# HC                0.45      0.24    -0.00     0.92 1.00     3812     3386
# HD                0.64      0.24     0.17     1.10 1.00     3625     3266
# HE                1.22      0.24     0.75     1.69 1.00     3307     2899


#Let's simulate to see that we can recover the parameters
nsim = 500 # simualations
...
for(i in 1:nsim) {
  ...
  # Save estimates for each category
  BAAHat[i] <- fixef(m)[1]
  BABHat[i] <- fixef(m)[1] + fixef(m)[3]
  BACHat[i] <- fixef(m)[1] + fixef(m)[4]
  BADHat[i] <- fixef(m)[1] + fixef(m)[5]
  BAEHat[i] <- fixef(m)[1] + fixef(m)[6]

  HAHat[i] <- fixef(m)[1]
  HBHat[i] <- fixef(m)[1] + fixef(m)[7]
  HCHat[i] <- fixef(m)[1] + fixef(m)[8]
  HDHat[i] <- fixef(m)[1] + fixef(m)[9]
  HEHat[i] <- fixef(m)[1] + fixef(m)[10]
}

# Check to see that we have recovered the parameters
round(c(
    c0 = mean(BAAHat), # 0.07
    c1 = mean(BABHat), # 0.1
    c2 = mean(BACHat), # -0.15
...
```{r, include=FALSE, eval=FALSE}
# A simple example to generate a predictor (Gaussian) for a Beta outcome
#beta <- 0.5
#n <- 100
#x1 <- rnorm(n)
...
# Below we do it for a categorical outcome.
# We simulate this 500 times to check we get accurate results.

# Set sample size. The lower, the more variability. n=250 seems nice
n <- 250

# Set effects for each of five categories
c0 <- 0.07
c1 <- 0.1
c2 <- -0.15
c3 <- -0.03
...
# Save all estimates so we check them after simulation.
aHat <- rep(NA, n)
bHat <- rep(NA, n)
cHat <- rep(NA, n)
dHat <- rep(NA, n)
...
# Five categories, each represented 20%
...
# Run model once so we can save the compiled model and reuse it
# instead of recompiling it at every iteration.
p <- get_prior(bf(y ~ x, phi ~ 1), data = d, family = Beta)
p[1,1] <- "normal(0,1)"
p[6,1] <- "normal(0,2)"
p[7,1] <- "normal(0,2)"
m <- brm(bf(y ~ x, phi ~ 1), data = d, prior = p, family = Beta, refresh = 0)

# now the above 500 times
for(i in 1:500) {
  x <- sample(x=c("A","B", "C", "D", "E"), 
              size=n, replace=TRUE, prob=rep(1/5, 5))
  m_x <- as.matrix(dummy_cols(x)[,-1])
