...
# https://stackoverflow.com/questions/1395309/how-to-make-r-use-all-processors
# https://nceas.github.io/oss-lessons/parallel-computing-in-r/parallel-computing-in-r.html
# run with doParallel instead
# cl <- makeCluster(detectCores() - 2)
# registerDoParallel(cl, cores = detectCores() - 2)

# I have 8 HP cores and 2
registerDoParallel(detectCores() - 2)

N <- seq(25, 250, 50) # Sample sizes for sim
...
# Simulate
# N = 25, 75, 125, 175, 225
l <- foreach(i = N, .combine = rbind) %:%
  # run 250 times to capture stochastic behavior
  foreach(j = 1:250, .combine = c) %dopar% {
    Ni <- eval(i)
    C <- rnorm(Ni)
    TS <- rpois(Ni, b_CTS * C + 5)
    E <- rnorm(Ni, b_C * C + b_TS * TS)
    d <- data.frame(C = C, TS = TS, E = E)

    m <- update(m, newdata = d, refresh = 0)
    fixef(m)[1] + fixef(m)[3]
  }
  # Each row is sample sizes (5 diff)
  # Each column is the number of executions 250
  # Average over this!
