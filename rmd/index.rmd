...
```{r, message=FALSE}
# Package names
packages <- c("ggplot2", "brms", "ggdag", "cmdstanr", "tipr", "broom", "broom.mixed", "tidyverse", "dagitty", "bayesfam")
...
for(i in 1:100) {
  ...
  #aHat[i] <- ranef(m)$x[1,1,1]
  #bHat[i] <- ranef(m)$x[2,1,1]
  #cHat[i] <- ranef(m)$x[3,1,1]
  #dHat[i] <- ranef(m)$x[4,1,1]
  #eHat[i] <- ranef(m)$x[5,1,1]
