...
```{r, include=FALSE}
# Package names
packages <- c("ggplot2", "brms", "ggdag", "cmdstanr", "tipr", "broom", "broom.mixed", "tidyverse", "dagitty", "rethinking", "foreach", "doParallel", "bayesplot")
...
N <- seq(10, 100, 5) # Sample sizes for sim
Nsim <- 250 # number of sim runs

# Set effects
b_CTS <- 1.5  # Confounder's effect on TS; vary this too
b_C <- 0.3    # Confounder's effect on outcome; vary this too
b_TS <- 0.2   # TS's effect on outcome; vary this too
...
# Simulate
l <- foreach(i = N, .combine = rbind) %:% # rows
  # run j times to capture stochastic behavior
  foreach(j = 1:Nsim, .combine = c) %dopar% { # columns
    Ni <- eval(i)
    C <- rnorm(Ni) # confounder
    TS <- rpois(Ni, 5 + b_CTS * C) # treatment affected by C+5
    E <- rnorm(Ni, b_C * C + b_TS * TS) # E affected by b_TS and b_C
