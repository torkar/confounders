...
In the plot above we see that the confounder, Skills, has a causal effect on the treatment, Lang, and on the outcome, Qual. Also, Lang has a direct causal effect on Qual, but that is what we're generally speaking interested in estimating (i.e., a treatment's effect on the outcome).
...
The SMD is the confounder's average effect on the different groups in Lang (e.g., Java and Python). In math, it's something like this:

$$\mathrm{SMD}[\mathrm{Skill}/\mathrm{Lang}] = \frac{\mathrm{mean}(\mathrm{Skill} \mid \mathrm{Python})}{\mathrm{sd}(\mathrm{Skill} \mid \mathrm{Python})}  - \frac{\mathrm{mean}(\mathrm{Skill} \mid \mathrm{Java})}{\mathrm{sd}(\mathrm{Skill} \mid \mathrm{Java})}$$

The difference in mean between Java and Python can then, by using SMD, be represented as a difference in $\sigma$. For example, a $3\sigma$, $1\sigma$, or $0.1\sigma$ difference between two categories (since we divide it by $\sigma_{\mathrm{Skills}}$).
...
tidy(with_skills, conf.int = TRUE) |>
  filter(term == "lang") |>
  pull(conf.low) |>
  tip_coef(confounder_outcome_effect = 1.5)
```
...
Assume we have som quality measure we're interested in distributed as $\mathrm{Normal}(45,5)$ (this could for example be the average number of bugs per $x$ lines of code introduced into a system by any language). Our scaled difference that we received above (i.e., $0.262$) can be transformed to a quality score by multiplying with $\sigma = 5$ (our assumed dispersion), i.e., $5 \cdot 0.262 \approx 1.3$. This implies that between two categories (Java and Python) we would need to see a difference of $\approx 1.3$ bugs in a quality score when implementing a feature. Is this unlikely? Perhaps; context matters. 
...
```{r, echo=FALSE, out.width="60%", fig.align="center", fig.cap="The difference between language on the outcome scale."}
tip_c <- tibble(x = sample(0:1, n, replace = TRUE)) |>
  mutate(qs = rnorm(n, mean = 45 + (0.262 * 5 * x), sd = 5))
...
tidy(with_skills, conf.int = TRUE) |>
  filter(term == "lang") |>
  pull(conf.low) |>
  tip_coef(exposure_confounder_effect = -2.3)
```
...
```{r udag, echo=FALSE, out.width="60%", fig.align="center", fig.cap="A larger more realistic DAG taken from Feldt et al. (2023)."}
var_labs <- c(
  `BA` = "BA", # business area
...
udag <- dagify(
  ...
  exposure = "TS",
  outcome = "E",
  coords = list(x = c(BA = 1, E = 6, H = 2, PL = 2, S = 4, PT = 4, L = 6, O = 7,
                      TS = 7, ST = 3),
                y = c(BA = 4, E = 4, H = 3, PL = 1, S = 9, PT = 7, L = 2, O = 1,
                      TS = 9, ST = 6))
)
...
```{r, echo=FALSE, out.width="60%", fig.align="center", fig.cap="The DAG where we have also marked in <span style='background-color: #D55E00;color: white;'>orange</span> the adjustment set, and numbered all edges."}
col_labs <- c(
  `BA` = "#56B4E9", `H` = "#56B4E9", `PL` = "#56B4E9",
...
1. It would not affect the estimate (**n/a**). This may happen because the chosen 
adjustment set _already_ blocks the confounding effect of a confounder, $C$.
...
Below is an updated UDAG where we mark all variables that we condition on in <span style="background-color: #D55E00;color: white;">orange</span>, the confounder is in <span style="background-color: #F0E442;color: white;">yellow</span>, while the outcome and treatment are in <span style="background-color: #000000;color: white;">black</span>. Other variables that we have no use for are now removed from the plot and further analysis. Already this step has clarified that there are a number of variables that we don't have to collect.

```{r, echo=FALSE, out.width="60%", fig.align="center", fig.cap="The DAG after closing backdoor paths and adjusting for possible confounders. Note that an umeasured confounder might still exist (in <span style='background-color: #F0E442;color: white;'>yellow</span>)"}
var_labs <- c(
  `BA` = "BA", # business area
  `O` = "O", # organisation
  `S` = "S", # size
...
To summarize, we have a challenge dealing with a confounder $C$, which affects both our treatment $TS$ and the outcome $E$, i.e., $TS \leftarrow C \rightarrow E$. Perhaps a researcher could swear that such a confounder cannot physically exist for the studied phenomena, but if one is not 100% sure, then we should do better and fall back on sensitivity analysis of unknown confounders. Before we do that, we need to set some assumptions about the effect size we are expecting. How large a direct effect could we anticipate, and is it likely that we can estimate that effect given a certain sample size?
...
```{r, echo=FALSE, out.width="60%", fig.align="center", fig.cap="The elementary DAG needing further analysis."}
my_dag <- dagify(
  TS ~ C,
  E ~ C,
  E ~ TS,
...
0. **We can recover the ground truth.** The wiggly black line converges around the red line quickly as we increase the sample size. That, in itself, is an indication that our model represents the underlying data generation process well (which is not strange in itself since the generative model and the inference model are similar).
...
* Make assumptions concerning SMD and estimate how large the confounder's effect (which is unknown or unmeasured) needs to be to cancel out the direct effect (0.3) of $TS$ on $E$.
...
```{r, echo=FALSE, out.width="60%", fig.align="center", warning=FALSE, fig.cap="Elementary DAG with assumptions."}
var_labs <- c(
  `TS` = "TS",
  `C` = "C",
  `E` = "E"
...
```{r, echo=FALSE, message=FALSE, fig.align="center", fig.cap="The interplay between the effect of a confounder on the outcome ($\\beta_{ce}$) and the scaled mean difference (SMD) in the confounders effect on the treatment."}
...
```{r, echo=FALSE, message=FALSE, fig.align="center", fig.cap="The relationship between $\\beta_{ce}$ and $\\mathrm{SMD}$."}
...
* effect sizes (where we exhaustively tried many variations for realistic effect sizes).
